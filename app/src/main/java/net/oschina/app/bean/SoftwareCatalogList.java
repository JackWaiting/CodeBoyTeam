...
public class SoftwareCatalogList extends Entity{
	...
	public static SoftwareCatalogList parse(InputStream inputStream)
		...
		try {
			...
			while (evtType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG:
					if (tag.equalsIgnoreCase("softwarecount")) {
						...
						if (tag.equalsIgnoreCase("name")) {
							...
							softwaretype.tag = StringUtils.toInt(
									xmlParser.nextText(), 0);
						}
				...
				case XmlPullParser.END_TAG:
					// 如果遇到标签结束，则把对象添加进集合中
					if (tag.equalsIgnoreCase("softwareType")
							&& softwaretype != null) {
						softwarecatalogList.getSoftwareTypelist().add(
								softwaretype);
						softwaretype = null;
					}
					break;
